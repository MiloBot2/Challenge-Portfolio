<html>
    <head>
        <title>Projects | Milo's Website</title>
        <link rel="stylesheet" href="http://localhost/milo/css/projects.css">
        <link href="http://localhost/milo/css/main.css" rel="stylesheet">
    </head>
    <body>
        <?php
            include("../includes/navbar.php");
        ?>
        <div class="page-wrapper">
            <div class="container"> 
                <div class="cards">
                    <div class='card'>
                        <div class='card-header'>
                            <p style='color: #36302b; font-size: 135%;'>Classic Client</p>
                        </div>
                        <div class='card-content'>
                            <img alt='Icon' width='90' height='90' src='https://i.redd.it/lhyfpzbifpo21.png'>
                            <p>Description: <span>Classic Client is een Minecraft cheat client die ik zelf heb gemaakt. Dit project is gemaakt in Java.</span></p>
                        </div>
                    </div>
                    <div class='card'>
                        <div class='card-header'>
                            <p style='color: #36302b; font-size: 135%;'>ProxyManager</p>
                        </div>
                        <div class='card-content'>
                            <img alt='Icon' width='90' height='90' src='https://proxy.vc/assets/About/Proxy_Logo_BlackWhite_2000px_RGB.jpg'>
                            <p>Description: <span>Deze tool scant en checkt proxies waarmee ik vervolgens kan gaan spelen op Minecraft voor maximale veiligheid.</span></p>
                        </div>
                    </div>
            </div>
            <div class="info">   
                    <span style="font-size: 140%;">Informatie</span>
                    <p>Dit zijn de meest recente projecten waar ik aan gewerkt heb. Hierbij heb ik verschillende talen gebruikt, waaronder dus Java en HTML.</p>
                </div>
        </div>
    </body>
</html>